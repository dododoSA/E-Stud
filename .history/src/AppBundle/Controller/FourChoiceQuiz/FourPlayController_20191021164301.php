...
class FourPlayController extends Controller {
    ...
    public function indexAction(Request $request, $four_course_id) {
        ...
        $session->set('selected_course_id', $four_course_id);
    ...
    function resultAction(Request $request, $four_course_id) {
        ...
        for ($i = 1; $i <= count($correct_choices); $i++) {
            $result[$i] = new Result;
            $result[$i]->setUserId(1);//////////////////////
            $result[$i]->setDate(date("Y-m-d"));
            $result[$i]->setFourQuizId($quizzes[$i]->getId());
            if ($correct_choices[$i] == $user_choices[$i]) {
                $result[$i]->setResult('correct');
            }
            else {
                $result[$i]->setResult('wrong');
            }
            $em->persist($result[$i]);
        }
