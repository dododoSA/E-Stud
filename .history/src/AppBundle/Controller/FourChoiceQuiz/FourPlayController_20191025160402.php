    ...
    public function quizAction(Request $request, $four_course_id, $quiz_num) {
        ...
        $form = $this->createFormBuilder()
            ->add('answer', ChoiceType::class, [
                'choices' => [//もっときれいに書きたいよね
                    '1.'.current(array_slice($choices, 0, 1, true)) => 1,
                    '2.'.current(array_slice($choices, 1, 1, true)) => 2,
                    '3.'.current(array_slice($choices, 1, 1, true)) => 3,
                    '4.'.current(array_slice($choices, 1, 1, true)) => 4,
                ],
                'expanded' => true
            ])
